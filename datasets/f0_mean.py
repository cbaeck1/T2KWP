...
def build_from_path(root_list, hparams, num_workers, tqdm=lambda x: x):
    for i in range(len(root_list)):
        speakers = glob.glob(os.path.join(root_list[i],'*'))
        speakers.sort()
        ...
        out_path = os.path.join(root_list[i], 'f0s.txt')
        write_metadata([future.result() for future in tqdm(futures)], out_path)
